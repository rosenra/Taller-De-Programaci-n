...
imprimirVectorRecursivo (v,dimL-1); //loop hasta que i=1 y ahi empieza a imprimir
writeln ('pos: ',dimL,'num: ', v[dimL]);
end; 
end;
...
function sumarParesR (v:vector; dimL:integer):integer;
begin
if (dimL >0) then
 begin
  if((v[dimL] mod 2)=0) then
    sumarParesR:= v[dimL]+ sumarParesR (v,dimL-1)
  else 
     sumarParesR:=sumarParesR(v, dimL -1);//!!!
 end
else if (dimL = 0) then
  sumarParesR:=0;
end;
...
max:integer;
begin
if (dimL = 1) then // = 1 pq cuando la recursion llegue a 1 toma ese como max y desp se actualiza
  ObtenerMaximo:= v[dimL]
else if (dimL > 0) then  //{-- Completar --} 
begin
  max:=ObtenerMaximo (v,dimL-1);// asi max se actualiza
  if (max < v[dimL]) then
  ObtenerMaximo:= v[dimL]
  else
  ObtenerMaximo:= max;
  ...
  if (dimL > 0) then
   begin
    if (valor = v[dimL] )then
      ...
      BuscarValor:=BuscarValor (v, dimL-1, valor);//no false, sino que 'minimizar' el vector
   end;
...
begin
  if n < 10 then
    write(n, ' ')  // Caso base: si el número es de un solo dígito, lo imprime
  else
  begin
    ImprimirDigitosNumero(n div 10);  // Llama recursivamente con el número sin su último dígito
    write(n mod 10, ' ');  // Imprime el último dígito
  end;
end;
var
i: integer; 
...
Begin 
  ...
  writeln;
  writeln;                   
 writeln( 'la sp: ',sumarParesR (v,dimL));//suma:= Sumar(v, dimL);
  writeln;
  writeln;
